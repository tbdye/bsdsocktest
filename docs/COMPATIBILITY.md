...
and are version-gated: entries apply only to versions at or below the tested
version. When a stack ships a fix and increments its version string, the
entries stop applying automatically.
...
## Roadshow (tested against 4.347 and 4.364)

Version string: `Roadshow 4.xxx` via `SBTC_RELEASESTRPTR`.
...
## UAE / Amiberry bsdsocket emulation (tested against 8.0.0)
